...
class Player(BaseBot):
    ...
    def __init__(self) -> None:
        ...
        self._pf_raises = 0          # per-hand raise counter, reset in on_hand_start

    def on_hand_start(self, game_info: GameInfo, current_state: PokerState) -> None:
        self.equity_cache = {}
        self._opp_bets_this_hand = 0
        self._pf_raises = 0          # how many times WE have raised pre-flop this hand
        self._update_mode()
    ...
    def _get_equity(
        ...
        if opp_revealed:
            # Revealed card info is highly valuable — use more iterations for
            # accuracy, especially on later streets where decisions are costly.
            iters = 200 if game_info.time_bank > 4.0 else 60
    ...
    def _auction_bid(self, eq: float, state: PokerState) -> ActionBid:
        """
        Auction bidding: beat TestBot with minimum necessary bid.

        TestBot's exact formula:
          eq > 0.65  -> min(pot * 1.5, chips, 400)   "strong" bid, capped at 400
          eq > 0.45  -> pot * 0.45                    "medium" bid
          else       -> pot * 0.1                     "weak"   bid

        We mirror their tier thresholds and bid exactly 1 chip above their
        ceiling for each tier.  Winning cheaply is critical: we pay our bid
        only when we LOSE; when we WIN, the loser (TestBot) pays their bid.

        So minimum-winning bids maximise info value per chip spent.
        """
        if eq < 0.28:
            # Too weak to benefit from info — skip the auction
            return ActionBid(0)
        ...
        if eq >= 0.65:
            # They'll bid their strong tier (up to 400) — beat with 401
            target = 401
        elif eq >= 0.45:
            # They'll bid medium (pot*0.45) — beat by a small margin
            target = int(pot * 0.45) + 6
        else:
            # They'll bid weak (pot*0.1) — beat cheaply
            target = int(pot * 0.1) + 6

        # Never commit more than 15% of remaining stack on auction alone
        bid = max(0, min(target, int(chips * 0.15)))
    ...
    def _preflop_action(self, eq: float, state: PokerState) -> ActionFold | ActionCall | ActionCheck | ActionRaise:
        ...
        # ── Hard folds vs escalating bet sizes ────────────────────────────
        if cost > 1000 and eq < 0.83:
            return ActionFold()
        if cost > 500 and eq < 0.75:
            return ActionFold()
        if cost > 200 and eq < 0.68:
            return ActionFold()
        if cost > 80 and eq < 0.64:          # tighter: fold more to 3-bets
            return ActionFold()

        # ── Raise logic — tiered by how deep into the betting war we are ──
        # raise_count tracks how many times WE have raised this hand
        raise_count = self._pf_raises

        if can_raise:
            min_r, max_r = state.raise_bounds

            if raise_count == 0:
                # ── Open raise OR BB squeeze vs limp ──────────────────────
                # Asymmetric sizing: big pots with premiums, small pots with
                # marginals — so losses stay small when we're wrong.
                if cost <= 20:
                    if eq >= 0.80:                   # premiums — charge max
                        size = int(pot * 3.5)
                        return ActionRaise(max(min_r, min(size, max_r)))
                    if eq >= 0.72:                   # strong hands
                        size = int(pot * 3.0)
                        return ActionRaise(max(min_r, min(size, max_r)))
                    if eq >= 0.60:                   # medium — moderate pot
                        size = int(pot * 2.2)
                        return ActionRaise(max(min_r, min(size, max_r)))
                    if eq >= 0.52:                   # marginals — keep pot small
                        size = int(pot * 1.75)
                        return ActionRaise(max(min_r, min(size, max_r)))
                else:
                    # Facing a 3-bet: only continue with genuinely strong hands
                    if eq >= 0.78:
                        size = int(pot * 3.0)
                        return ActionRaise(max(min_r, min(size, max_r)))

            elif raise_count == 1:
                # ── 4-bet: we already raised once, they 3-bet us ──────────
                # Only go to war with near-premium holdings
                if eq >= 0.85:
                    size = int(pot * 2.5)
                    return ActionRaise(max(min_r, min(size, max_r)))

            # raise_count >= 2: no 5-bet+ — call or fold below

        # ── Call / check / fold ───────────────────────────────────────────
        if cost > 40 and eq < 0.50:
            return ActionFold()
        if cost > 20 and eq < 0.46:
            return ActionFold()
    ...
    def _postflop_action(
        ...
        if cost > 0:
            ...
            # ── SPR gate: large pot + weak hand = cut losses now ──────────
            # When little stack remains relative to pot, opponent's bet is
            # effectively an all-in threat — only commit with strong hands.
            if spr < 1.5:
                if h_type in ("high card", "pair") and adj_eq < 0.82:
                    return ActionFold()
                if h_type == "two pair" and adj_eq < 0.72:
                    return ActionFold()
                if h_type in ("straight", "three of a kind") and adj_eq < 0.62:
                    return ActionFold()

            # ── Three-street bet: opponent has a real hand, not a bluff ───
            if self._opp_bets_this_hand >= 3 and h_type not in (
    ...
    def _facing_bet(
        ...
        if h_type in ("flush", "full house", "four of a kind", "straight flush"):
            call_thresh = max(pot_odds, 0.42)
        elif h_type in ("straight", "three of a kind"):
            call_thresh = max(pot_odds + 0.06, 0.50)   # tightened: was 0.03 / 0.45
        elif h_type == "two pair":
            call_thresh = max(pot_odds + 0.10, 0.54)   # tightened: was 0.05 / 0.48
        else:
            implied_boost = 0.03 if spr > 3 else 0.0
            call_thresh = pot_odds + 0.12 + implied_boost  # tightened: was 0.08

        if self._opp_bets_this_hand >= 2:
            call_thresh += 0.12  # second street bet is serious — was 0.08

        if adj_eq >= call_thresh:
            if adj_eq >= 0.88 and can_raise and cost < pot * 0.8 and spr > 2:  # was 0.82
                min_r, max_r = state.raise_bounds
    ...
    def _river_facing_bet(
        ...
        """River-specific calling/folding logic."""
        opp_revealed = state.opp_revealed_cards or []

        if opp_revealed:
            # We won the auction and know one of their hole cards.
            # The MC equity (adj_eq) is already computed with that card — trust
            # it directly rather than using a blind hand-category table.
            effective_rate = max(adj_eq - 0.04, 0.0)  # tiny safety margin
        else:
            # No revealed card — use the conservative empirical table.
            ...
            if bet_ratio > 1.0:
                ...
                empirical *= 0.90
            effective_rate = empirical

        if effective_rate < 0.15:
            return ActionFold()

        if pot_odds >= effective_rate:
            return ActionFold()
    ...
    def _checked_to_us(
        ...
        if adj_eq >= 0.92:
            # Monster — overbet to build the largest possible pot
            size = int(pot * BET_OVERBET)
            return ActionRaise(max(min_r, min(size, max_r)))

        if adj_eq >= 0.88:
            # Very strong — bet big, occasionally slow-play to balance range
            if not is_river and random.random() < 0.15:
                return ActionCheck()
            size = int(pot * BET_LARGE)   # was BET_MEDIUM (0.55 → 0.75)
            return ActionRaise(max(min_r, min(size, max_r)))

        if adj_eq >= 0.70:
            # Strong hand — always bet medium on every street
            size = int(pot * BET_MEDIUM)  # river was BET_SMALL (0.33 → 0.55)
            return ActionRaise(max(min_r, min(size, max_r)))
    ...
    def get_move(
        ...
        if current_state.street == "pre-flop":
            action = self._preflop_action(raw_eq, current_state)
