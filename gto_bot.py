...
class Player(BaseBot):
    ...
    def _adjust_equity(self, raw_eq: float, info: dict, cost: int, pot: int) -> float:
        ...
        # Behavioral adjustments needed very badly , cause i can exploit
        if cost > 0:
        ...
        if cost > 0 and self._opp_bet_sizes:
    ...
    def _auction_bid(self, eq: float, state: PokerState) -> ActionBid:
        ...
        if eq >= 0.75:
            bid = int(pot * random.uniform(0.8, 1.4))
        elif eq >= 0.55:
            bid = int(pot * random.uniform(0.4, 0.8))
        elif eq >= 0.40:
            bid = int(pot * random.uniform(0.15, 0.40))
        else:
            bid = int(pot * random.uniform(0.05, 0.20))
    ...
    def _preflop_action(self, eq: float, state: PokerState) -> ActionFold | ActionCall | ActionCheck | ActionRaise:
        ...
        # thresholds so i can choose if i want to ffold 
    ...
    def _postflop_action(
        ...
        if cost > 0:
            ...
            # Asymmetric info: fold high card when opponent knows our cards and bets
            if self._opp_has_info and h_type == "high card":
                return ActionFold()
